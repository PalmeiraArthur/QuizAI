...
const CreateUser = () => {
  ...
  const { containerRef, handleMouseMove } = useShinyEffect(); // ✨ Adiciona só isso
  ...
  return (
    <BackgroundPattern>
      <div className="flex flex-col gap-18 items-center justify-center min-h-screen">

        <img src="/quizAiTransparente.svg" alt="Logo" className="w-90 h-auto" />

        <div ref={containerRef} onMouseMove={handleMouseMove} className="shiny-container w-[90vw] max-w-[600px] bg-russianViolet rounded-md shadow-padrao px-8 py-12 flex flex-col gap-5">
          ...
          <form onSubmit={handleSubmit} className=" space-y-5">
            <div>
              <input
                ...
                maxLength={30}
                disabled={loading}
                className="w-full px-4 py-3 bg-raisinBlack text-white border border-none rounded-lg focus:outline-none focus:ring-1 focus:ring-pistachio focus:border-transparent disabled:opacity-50"
